...
export async function GET(
  ...
  const { dictamenId } = await context.params;
  ...
  const backendUrl = buildBackendUrl(`/dictamen/${encodeURIComponent(dictamenId)}/descargar`);
  ...
  if (!backendResponse.ok) {
    let detail = "No se pudo descargar el dictamen.";
    try {
      ...
      // Ignorado: la respuesta puede no venir en JSON.
    }
